...
export function SeverityBadge({severity}: {severity: ForensicEvent['severity']}) {
    ...
    return <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${style}`}>{severity}</span>;
}
...
export function MlLabelBadge({label}: {label: ForensicEvent['ml_label']}) {
    ...
    return <span className={`text-xs font-mono font-semibold px-2 py-0.5 rounded-md ${style}`}>{label}</span>;
}
...
export default function ForensicEvents() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const data: ForensicEvent[] = Array.isArray(raw) ? raw : Array.isArray(raw.events) ? raw.events : Array.isArray(raw.data) ? raw.data : [];
                setEvents(data);
            } catch (err: unknown) {
    ...
    return (
        <>
                        ...
                        <tbody>
                            {events.map((event) => (
                                ...
                                >
                                    ...
                                    <td>
                                        <ConfidenceBar value={event.ml_confidence} />
                                    </td>
                                    <td>
                                        <ConfidenceBar value={event.p_malicious} />
                                    </td>
                                    <td>
                                        {event.regex_match ? (
            ...
            {isDetailModalOpen && selectedEvent && <ModalForensicDetails event={selectedEvent} onClose={() => setIsDetailModalOpen(false)} />}
        </>
    );
}
